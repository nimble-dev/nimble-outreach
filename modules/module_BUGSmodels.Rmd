...
library(nimble)
read_chunk('module_BUGSmodels.R')
```


# The BUGS language

The BUGS language is described in detail in the [relevant section of the BUGS manual](http://www.openbugs.net/Manuals/ModelSpecification.htmlBUGS). Information on how Nimble's 'dialect' of BUGS differs from the original and information on the distributions and functions provided in Nimble can be found in Section 5 of the [Nimble Users Manual](http://r-nimble.org/manuals/NimbleUserManual.pdf).

Basically, one specifies the variables in the hierarchical model as lines of code.
 -  stochastic nodes (data and parameters) are indicated with `~` and 
 - deterministic nodes (which are deterministic functions of other nodes and of constants) are indicated by `<-`.

There are lots of example models from the classic BUGS examples provided in the classic-bugs directory of the installed Nimble R package or from the [JAGS website](https://sourceforge.net/projects/mcmc-jags/files/Examples/).  Information about the models can be found in [Volume 1](http://www.mrc-bsu.cam.ac.uk/wp-content/uploads/WinBUGS_Vol1.pdf), [Volueme 2][http://www.mrc-bsu.cam.ac.uk/wp-content/uploads/WinBUGS_Vol2.pdf], and [Volume 3][http://www.mrc-bsu.cam.ac.uk/wp-content/uploads/WinBUGS_Vol3.pdf] of the WinBUGS example manuals.
...
This model has Poisson distributed data, $x_i \sim \mbox{Poi}(\lambda)$, with mean $\lambda_i = \theta_i \cdot t_i$, and random effects, $\theta_i$, and hyperparameters, $\alpha$ and $\beta$ of the random effects distribution.
...
If all you want to do is run an MCMC, this might not be so interesting to you. But by providing an explicit model object, we allow you to operate the model and program with it.
...
We have control over the model. Let's change values in the model and recalculate the density values.

```{r}
pump$alpha <- 0.5
pump$calculate('theta')   # or calculate(pump, 'theta')
...
pump$theta
getLogProb(pump, 'theta')  # equivalent to pump$logProb_theta
pump$calculate('theta')
getLogProb(pump, 'theta')  # equivalent to pump$logProb_theta
pump$lambda
...
**Question**: What uses might there be for being able to change the data values and change whether a node is treated as a fixed data value or an unknown parameter?

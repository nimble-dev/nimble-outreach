...
conf <- configureMCMC(model, thin = 10)
...
library(coda)
effectiveSize(as.mcmc(smp_basic))
```


# Blocking the hyperparameters

Given the posterior correlation between the hyperparameters for each group, an obvious first step is to try a block samplers on $(a_1,b_1)$ and $(a_2, b_2)$.
...
library(coda)
effectiveSize(as.mcmc(smp_hyp_block))
```

# Blocking the random effects too

But perhaps we should have blocked the hyperparameters with their dependent random effects. 

```{r, effects-block, fig.height=5, fig.width=12}
conf$removeSamplers(c('a', 'b', 'p'))
gp1nodes <- model$getDependencies(c('a[1]', 'b[1]'), stochOnly = TRUE)
gp2nodes <- model$getDependencies(c('a[2]', 'b[2]'), stochOnly = TRUE)
conf$addSampler(gp1nodes, 'RW_block')
conf$addSampler(gp2nodes, 'RW_block')
...
effectiveSize(as.mcmc(smp_full_block))
```
...
confAuto <- auto$spec  # 'spec' is a holdever from a previous version of R and will be changed to 'conf' shortly
...
Actually, given that the module on user-defined distributions provides a Dirichlet-multinomial distribution and the beta-binomial distribution is the special case of that with only two categories, we could just borrow that code.
...
effectiveSize(as.mcmc(smp_cross))
```
...
out # after 100
#       a[1]       a[2]       b[1]       b[2] 
# 41.0924074  1.5832673  4.6905697  0.5552738 
```
...
Here we use Nimble's alternative parameterizations to directly represent the beta distribution in terms of mean and standard deviation. We need to be a little bit careful about starting values and constraints in order to get the optimization to converge.
...
inits <- list( mu = c(.8, .5), sigma = c(.1, .1))
modelReparam <- nimbleModel(codeReparam, constants = consts, data = data, inits = inits)
box = list( list('mu', c(.1, .98)), list('sigma', c(0, .3)))  
modelReparam$simulate('p')
mcemReparam <- buildMCEM(modelReparam, latentNodes = 'p', burnIn = 500, boxConstraints = box)
outReparam <- mcemReparam(maxit = 50)
# outReparam 
```
need to do again with taking box out from Nelder-Mead
TRY THIS with original lbfgsb

At the moment this is not working because *optim()* doesn't like getting `-Inf` as the value of the objective function, and because there is something strange going on with the steps in the BFGS optimization. This should be addressed in the next version of Nimble.

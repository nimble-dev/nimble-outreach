...
include:
   ...
   - Ability to do two core operations:
      ...
      + *simulate()* 
	  +  *getParam()* 
   - Ability to do two other helpful operations
      + *getLobProb()* 
      + *calculateDiff()*
...
An important distinction is between *nodes* and *variables*.
 ...
 - variable: a collection of one or more nodes with the same name, e.g., `r` or `y` in the pump model 
...
*The spaces after the commas are important*, but you will usually be
 obtaining node names from a model object, so they will be correct.
...
*Calculate* means:
   ...
   - For a deterministic node, calculating the nodes value

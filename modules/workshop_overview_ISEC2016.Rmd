...
Here are the modules we'll work through for this workshop, though we probably won't get to all of them.
 ...
 - Introduction to NIMBLE (using a population state-space model)
    ...
    4. [Customizing an MCMC](customizing_mcmc_state_space_example_slides.html) BROKEN (This example is a bit finicky and illustrates issues about how the model is written that we may explore later.)
 ...
 - NIMBLE programming: models and algorithms
    ...
    8. [Programming with models (II): Understanding and querying model structure](model_structure_state_space_example_slides.html) CLEAN UP DRAWING CODE 
    9. If time: how to re-write this state-space example to allow more efficient MCMC (by NIMBLE or JAGS). TO-DO.
    10. nimbleFunction programming (I): Compiling simple math from R to C++.
        [Providing new distributions for use in BUGS](user_dist_slides.html)
    11. [Capture-recapture example: a user-defined distribution for multi-state capture histories](user_dist_DHMM_slides.html)
    12. nimbleFunction programming (II): model-generic two-stage programming.
    13. Spatial capture-recapture example: Writing new MCMC samplers
 - Writing and using other algorithms:
    14. A general model simulator
    15. Maximum likelihood by ascent-based Monte Carlo Expectation Maximization
    16. Sequential Monte Carlo (aka particle filtering).
    17. Comparing MCMCs

Reparameterized version not working - giving Inf values in l-bfgs-b; need to figure out what is going on - Cmodel seems to be getting rep(.99999,4) as vals for maxNodes somehow
...
Here we use NIMBLE's alternative parameterizations to directly represent the beta distribution in terms of mean and standard deviation. Convergence appears to be faster to the MLE of $\hat\mu=0.898$ and $\hat\sigma=0.043$, but we'd need to investigate further. 
...
mcemReparam <- buildMCEM(modelReparam, latentNodes = latents,
                         boxConstraints = list(list(c('mu[1]', 
                                        'mu[2]', 'sigma[1]', 'sigma[2]'),
                                        c(0, 1))))
print(system.time(

Defining one's own MCMC samplers
==========================
...
library(nimble)
#read_chunk('module_user_sampler.R')
```
...
Let's look at the code for the Metropolis sampler. You can find this in the file *R/MCMC_samplers.R* in the source code for the Nimble package (note that ```nimble:::sampler_RW``` shows a modified version of this -- the nimbleFunction after it has been built. 
...
The *run* function for the reflection sampler needs to check for the proposed value against the distribution bounds and modify the proposal as needed.

However, we first need to modify the *setup* function to check if the distribution has finite lower or upper bounds and only consider scalar parameters, thereby avoiding some computation at run-time. We can use 
...
In this case, we could have used a conjugate sampler, which would automatically respect the lower bound of zero, but for illustration let's compare a standard metropolis sampler with the new reflection sampler. 

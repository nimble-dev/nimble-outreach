...
In this module we will:
   ...
   - Use the house martin state-space model from K&egrave;ry and Schaub (2012).
...
```{r, ssm-mcmc2}
mcmcConf2 <- configureMCMC(ssm) # can skip mcmcConf
...
MCMCdefs <- list(custom = quote({
    ...
    mcmcConf2$addSampler(target = 'sigma.obs', type = 'RW', control = list(log=TRUE))
    mcmcConf2$addSampler(target = 'sigma.proc', type = 'RW', control = list(log=TRUE))
    mcmcConf2$addSampler(target = Rmodel$expandNodeNames('r'), type = 'RW_block')
    mcmcConf2
}))
